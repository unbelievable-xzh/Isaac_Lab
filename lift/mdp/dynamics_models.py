...
class DynamicsModel:
    ...
    def update(self, prev_state: Tensor, prev_action: Tensor, prev_history: Tensor, next_state: Tensor) -> None:
        ...
        inference_ctx_factory = getattr(torch, "inference_mode", None)
        is_inference_enabled = getattr(torch, "is_inference_mode_enabled", None)

        if inference_ctx_factory is not None and callable(inference_ctx_factory):
            should_disable_inference = False
            if callable(is_inference_enabled):
                try:
                    should_disable_inference = bool(is_inference_enabled())
                except RuntimeError:
                    # ``torch.is_inference_mode_enabled`` can raise if the build
                    # lacks support; fall back to conservatively disabling.
                    should_disable_inference = True
            else:
                # Older PyTorch releases expose ``torch.inference_mode`` but not
                # ``torch.is_inference_mode_enabled``.  Conservatively assume
                # inference mode is active because the observation stack might
                # have enabled it.
                should_disable_inference = True
            inference_guard = (
                inference_ctx_factory(False) if should_disable_inference else nullcontext()
            )
        else:
            inference_guard = nullcontext()
